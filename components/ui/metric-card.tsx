  ...
  return (
    <div className="card group relative overflow-hidden flex flex-col gap-5">
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-baseline gap-2 min-h-[36px]">
          {loading ? (
            ...
            <h3 className="text-3xl font-black text-main italic tracking-tighter">{value}</h3>
          )}
        </div>
      ...
      <div className="h-1 w-full bg-white/[0.03] rounded-full overflow-hidden relative z-10">
        <div
          className={`h-full rounded-full transition-all duration-[2000ms] ease-out shadow-[0_0_10px_currentColor] ${loading ? "animate-pulse" : ""}`}
          style={{ width: loading ? "30%" : "65%", backgroundColor: color, color: color }}
        ></div>
      </div>
    </div>
  );
