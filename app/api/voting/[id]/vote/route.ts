...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    await db().runTransaction(async (tx) => {
      ...
      const voteRef = votingRef.collection("votes").doc(user.uid);
      ...
      patch[decision] = admin.firestore.FieldValue.increment(1);

      tx.set(votingRef, patch, { merge: true });
      tx.set(voteRef, {
        userId: user.uid,
        email: user.email,
        decision,
    ...
    return error("vote_failed", 500, { detail: message });
  }
}
